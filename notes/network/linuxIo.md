...
原文：[https://www.jianshu.com/p/486b0965c296](https://www.jianshu.com/p/486b0965c296)
...
![](/img/notes/network/linuxIo/io_model_matrix.png)
...
![](/img/notes/network/linuxIo/bio.png)
...
![](/img/notes/network/linuxIo/nio.png)
...
![](/img/notes/network/linuxIo/io_multiplexing.png)
...
同步与异步的根本性区别，同步是需要主动等待消息通知，而异步则是被动接收消息通知，通过回调、通知、状态等方式来被动获取消息。IO多路复用在阻塞到select阶段时，用户进程是主动等待并调用select函数获取数据就绪状态消息，并且其进程状态为阻塞。所以，把IO多路复用归为同步阻塞模式。

## 2.4 信号驱动式IO（signal-driven IO）

信号驱动式I/O：首先我们允许Socket进行信号驱动IO,并安装一个信号处理函数，进程继续运行并不阻塞。当数据准备好时，进程会收到一个SIGIO信号，可以在信号处理函数中调用I/O操作函数处理数据。过程如下图所示：

![](/img/notes/network/linuxIo/signal_driven.png)
...
![](/img/notes/network/linuxIo/aio.png)
...
![](/img/notes/network/linuxIo/io_compare.png)
